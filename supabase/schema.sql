...
create table if not exists public.trading_rules (
  ...
  max_risk_percent numeric(5,2) not null default 0 check (max_risk_percent >= 0),
  min_rr numeric(6,2) not null default 0 check (min_rr >= 0),
  allowed_sessions text[] not null default '{}',
  allowed_pairs text[] not null default '{}',
  allowed_directions text[] not null default '{}',
...
create index if not exists feedback_reports_type_created_idx on public.feedback_reports(type, created_at desc);
